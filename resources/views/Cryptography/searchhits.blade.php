...
<!-- The Modal -->
	<div id="modal" class="modal" style="display:none">
	  ...
	  <div class="modal-content" style="width:60%;margin: auto;">
		...
		<h4>File Data Content</h4>
			<div id="filedata"></div>
		<hr>
		<small style="font-size:10px;margin-top:0px;float:right"><a id="download">
		</a></small>
	  </div>
	</div>
	<!-- **************************************************************************** -->
	<div class="flex-container">
		...
		<div class="row"> 
			<br>
			<div style="font-weight:bold;font-size:20px;line-height: 50px;height:50px;background-color:#0000AA;color:white;" class="flex-item"> 
			<img style="float:left;" height="50px" src="{{ asset('apps/ishipment/images/mentor.png') }}"></img>
			<div style="margin-right:150px;">OSS Governance [Search - File - <span style=""> <a id="download1">{{ $package }}</a>]</span></div>
			</div>
			<div class="flex-item"> 
			<small class="flex-item" style="font-size:12px;"><a id="" href="#"></a></small>
			</div>
			<hr>
					<label for="bulktriage">Triage:</label>
					<select name="bulktriage" id="bulktriage">
						<option value="Valid">Valid</option>
						<option value="Ignore">Ignore</option>
					</select>
					
					<label for="bulkcomment">Comment:</label>
					<input type="text" id="bulkcomment" name="bulkcomment">
					<button id="bulkupdate">Bulk update</button>
			<br><br>
			<div class="flex-item"> 
	...
	$(document).ready(function()
	{
		...
		var url = null;//'/oss/file/'+project.name+"/"+file_identifier;
		console.log(url);
		...
		{	
			//bulkcomment
			//bulktriage
			//console.log($("#bulktriage option:selected" ).text();
			var comment = $('#bulkcomment').val();
			var text = $("#bulktriage option:selected" ).text();
			...
			for(var i=0;i<rows.length;i++)
			{
				...
				if((data.triage.text == '')||(data.triage.text == null))
				{
					rows[i].update({"triage":{"comment":comment,"text":text}});
					var status = {};
					status.id = data.id;
					status.text = text;
					...
					UpdateTriageStatus(cells[0],status);
				}
			}
			//	hits[i].triage.text = 'Ignore';
